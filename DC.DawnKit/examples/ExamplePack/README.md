...
- Top-level fields the loader reads: `pack` (display name, spaces allowed —
  ...
  value: `1000 + (low − 700,000,000) / 100`, here `377734`), and the content
  arrays `cards` / `weapons` / `weaponPowers`. There are **no**
  `schemaVersion`/`author` manifest fields yet (a semver/capability handshake
  is an open SPEC question); the set's author defaults to the pack name, and
