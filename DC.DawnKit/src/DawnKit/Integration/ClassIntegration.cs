...
{
    /// <summary>
    /// Profession.weapons / Profession.talents / Profession.startingCards
    /// attachment (WEAPON-SPEC §5.3, P18).
    /// The character-creation UI reads those lists live — no UI patching
    /// (LoadWeapons reads activeProfession.weapons; LoadStartingCards reads
    /// activeProfession.startingCards, CharacterBuilder.cs:1240).
    ...
    {
        /// <summary>
        /// Appends this owner's injected weapons/starting cards/talents to the live
        /// Profession assets. Returns the distinct class names that carry at least
        /// one attachment.
        /// </summary>
        internal static List<string> AttachOwner(string owner)
        {
            var attached = new List<string>();
            bool anyContent = Registry.Cards.Any(t => t.Spec.Kind != CardKind.Card && t.Spec.Owner == owner && t.Card != null) ||
            ...
            foreach (CardRegistration t in Registry.Cards)
            {
                if (!t.Spec.IsStartingCard || t.Spec.Owner != owner || t.Card == null)
                {
                    continue;
                }
                foreach (Profession prof in ResolveClasses(owner, t.Card.name, t.Spec.Classes))
                {
                    if (prof.startingCards == null)
                    {
                        prof.startingCards = new List<Card>();
                    }
                    if (!prof.startingCards.Contains(t.Card))
                    {
                        prof.startingCards.RemoveAll(c => c != null &&
                            (c.cardID == t.Card.cardID ||
                             string.Equals(c.name, t.Card.name, StringComparison.OrdinalIgnoreCase)));
                        prof.startingCards.Add(t.Card);
                    }
                    if (!attached.Contains(prof.name))
                    {
                        attached.Add(prof.name);
        ...
        /// <summary>Debug dump: per-class weapon/talent/starting-card list sizes after
        /// injection, so live verification can confirm Profession lists actually grew.</summary>
        internal static void LogClassCounts()
        {
            if (AssetManager.allClasses == null || AssetManager.allClasses.Count == 0 ||
                (!Registry.Cards.Any(t => t.Spec.Kind != CardKind.Card) && Registry.Talents.Count == 0))
            {
                return;
            }
            DawnKitPlugin.Log.LogInfo("[DawnKit] Class weapon/talent/starting-card counts: " + string.Join(", ",
                AssetManager.allClasses.Where(p => p != null).Select(p =>
                    $"{p.name}={(p.weapons?.Count ?? 0)}w/{(p.talents?.Count ?? 0)}t/{(p.startingCards?.Count ?? 0)}s")));
        }
    }
}
